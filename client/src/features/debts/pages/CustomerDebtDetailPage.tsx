...
import { DollarOutlined, ArrowLeftOutlined, PhoneOutlined, MailOutlined, EnvironmentOutlined, WarningOutlined, SearchOutlined, UnorderedListOutlined, HistoryOutlined } from '@ant-design/icons';
...
const CustomerDebtDetailPage: React.FC = () => {
  ...
  const invoiceColumns: any[] = [
    ...
    },
    {
      ...
      render: (_: any, rec: any) => rec.sales_order ? (
        <Button type="link" size="small" style={{ padding: 0 }} onClick={() => navigate(`/sales-orders/${rec.sales_order.id}`)}>
          {rec.sales_order.order_code}
        </Button>
      ) : '-',
    },
    {
    ...
    },
    {
      ...
      render: (_: any, rec: any) => (
        <Space size="small">
          {rec.sales_order?.items?.length > 0 && (
  ...
  return (
    <div>
      ...
      <Card title={t('debt.invoiceList') + ` (${filteredReceivables.length})`} style={cardStyle}>
        ...
        <Table
          ...
          pagination={{
            current: page, pageSize, total: filteredReceivables.length, showSizeChanger: true,
      ...
      >
        {modalData?.type === 'items' && (() => {
          ...
          const items = rec.sales_order?.items || [];
          return (
            <>
              {rec.sales_order?.expected_delivery && (
                <Text type="secondary" style={{ fontSize: 12, display: 'block', marginBottom: 4 }}>{t('order.expectedDelivery')}: {formatDate(rec.sales_order.expected_delivery)}</Text>
              )}
              {rec.sales_order?.notes && (
                <Text type="secondary" style={{ fontSize: 12, display: 'block', marginBottom: 8 }}>{t('common.notes')}: {rec.sales_order.notes}</Text>
              )}
              <Table
                size="small"
                dataSource={items}
                ...
                summary={() => rec.sales_order?.grand_total != null ? (
                  <Table.Summary.Row>
                    <Table.Summary.Cell index={0} colSpan={4} align="right"><Text strong>{t('order.orderTotal')}</Text></Table.Summary.Cell>
                    <Table.Summary.Cell index={1} align="right"><Text strong style={{ color: '#1890ff' }}>{formatVND(rec.sales_order.grand_total)}</Text></Table.Summary.Cell>
                  </Table.Summary.Row>
                ) : undefined}
              />
            </>
