...
import { Card, Table, Typography, Space, Row, Col, Statistic, Spin, Empty, Button, Tooltip, Tag } from 'antd';
import { DollarOutlined, ArrowLeftOutlined, EyeOutlined, UserOutlined, PhoneOutlined, MailOutlined } from '@ant-design/icons';
...
const CustomerDebtDetailPage: React.FC = () => {
  ...
  const invoiceColumns: ColumnsType<any> = [
    ...
    { title: t('debt.invoiceNumber'), dataIndex: 'invoice_number', key: 'inv', width: 140, ellipsis: true },
    {
      ...
      render: (_: any, r: any) => r.sales_order ? (
        <Button type="link" size="small" style={{ padding: 0 }} onClick={() => navigate(`/sales-orders/${r.sales_order.id}`)}>
          {r.sales_order.order_code}
        </Button>
      ) : '-',
    },
  ...
  // Payment history from all receivables
  const allPayments = receivables
    .flatMap((r: any) => (r.payments || []).map((p: any) => ({ ...p, invoice_number: r.invoice_number })))
    ...
    <div>
      ...
      <Card style={cardStyle}>
        ...
        <Row gutter={[12, 12]}>
          ...
          {customer.address && <Col xs={24} sm={8}><div style={fieldStyle}><Text style={labelStyle}>{t('customer.address')}</Text><Text strong>{customer.address}</Text></div></Col>}
        </Row>
      </Card>

      {/* Invoice list */}
      <Card title={t('debt.invoiceList') + ` (${receivables.length})`} style={cardStyle}>
        <Table columns={invoiceColumns} dataSource={receivables} rowKey="id" pagination={false} size="small" scroll={{ x: 900 }} />
      </Card>
