...
const CustomerDebtDetailPage: React.FC = () => {
  ...
  const filteredReceivables = useMemo(() => {
    ...
    if (search) {
      ...
      list = list.filter((r: any) => r.invoice_number?.toLowerCase().includes(q) || r.sales_order?.order_code?.toLowerCase().includes(q));
    }
    if (onlyUnpaid) list = list.filter((r: any) => Number(r.remaining) > 0);
  ...
  const totals = useMemo(() => filteredReceivables.reduce((acc, r: any) => ({
  ...
  }), { original: 0, paid: 0, remaining: 0 }), [filteredReceivables]);

  const openingBalance = useMemo(() => {
    if (!detail || !invoiceDateRange?.[0]) return 0;
    const from = invoiceDateRange[0].startOf('day');
    return (detail.receivables as any[]).filter((r: any) => dayjs(r.invoice_date).isBefore(from, 'day'))
  ...
  const chartData = useMemo(() => {
    ...
    for (const r of detail.receivables as any[]) {
  ...
  const handleExport = async (action: 'excel' | 'pdf' | 'preview' | 'preview-excel', range: { from_date?: string; to_date?: string }) => {
    ...
    try {
      if (action === 'preview') {
        ...
        const res = action === 'pdf' ? await receivableApi.exportPdf(customerId, range) : await receivableApi.exportExcel(customerId, range);
  ...
  const initials = getInitials(customer.company_name || customer.contact_name || 'KH');
  ...
  const invoiceColumns: any[] = [
    ...
    { title: t('order.orderCode'), key: 'order_code', width: 160, render: (_: any, rec: any) => rec.sales_order ? <Button type="link" size="small" style={{ padding: 0 }} onClick={() => navigate(`/sales-orders/${rec.sales_order.id}`)}>{rec.sales_order.order_code}</Button> : '-' },
    ...
    { title: '', key: 'actions', width: 50, align: 'center' as const, render: (_: any, rec: any) => rec.sales_order?.items?.length > 0 ? <Tooltip title={t('order.productDetails')}><Button type="text" size="small" icon={<UnorderedListOutlined />} onClick={() => setModalData({ type: 'items', record: rec })} /></Tooltip> : null },
  ];
    ...
    <div className="pd-root">
      <div className="pd-topbar">
        <div className="pd-breadcrumb">
          <a onClick={() => navigate('/debts')}>{t('debt.receivables')}</a>
          <span className="sep">/</span>
          <span className="current">{customer.company_name || customer.contact_name}</span>
        </div>
      ...
      <div className="pd-main">
        <section className="pd-hero pd-hero-cust">
          <div className="pd-cust-card">
            <div className="pd-cust-cover"><div className="pd-cust-cover-label">{t('debt.receivables')}</div></div>
            <div className="pd-cust-avatar-wrap"><div className="pd-cust-avatar">{initials}</div></div>
            <div className="pd-cust-body">
              <h2 className="pd-cust-name">{customer.company_name || customer.contact_name}</h2>
              <div className="pd-cust-id">{customer.id?.slice(0, 8).toUpperCase()}</div>
              ...
              <ul className="pd-cust-meta-list">
                {customer.contact_name && customer.company_name && <li><span className="lbl">{t('customer.contactName')}</span><span className="val">{customer.contact_name}</span></li>}
                {customer.phone && <li><span className="lbl">{t('customer.phone')}</span><span className="val mono">{customer.phone}</span></li>}
                {customer.email && <li><span className="lbl">Email</span><span className="val" style={{ fontSize: 12 }}>{customer.email}</span></li>}
              </ul>
              <div className="pd-cust-actions">
                <button className="pd-btn" onClick={() => navigate(`/customers/${customer.id}`)}>{t('common.viewDetail')}</button>
              </div>
            </div>
          </div>
          ...
          <div className="pd-info">
            <div className="pd-meta-row">
              <span className="pd-badge pd-badge-info">{t('debt.receivableLedger')}</span>
            </div>
            <div className="pd-title-block">
              <h1 className="pd-title">{t('debt.totalDebt')}</h1>
              <p className="pd-subtitle">{t('debt.ledgerSubtitle')}</p>
            </div>
            ...
            <div className="pd-metrics">
              <div className="pd-metric">
                ...
                <div className="pd-metric-trend">{detail.receivables?.length || 0} {t('debt.invoices')}</div>
              </div>
              <div className="pd-metric">
          ...
          <div className="pd-tabs-wrap" style={{ padding: 24, marginBottom: 16 }}>
            ...
            <ResponsiveContainer width="100%" height={260}>
              <AreaChart data={chartData} margin={{ top: 5, right: 20, left: 0, bottom: 5 }}>
                <defs><linearGradient id="cdRem" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#B91C1C" stopOpacity={0.2} /><stop offset="95%" stopColor="#B91C1C" stopOpacity={0} /></linearGradient></defs>
                ...
                <Area type="monotone" dataKey="remaining" name={t('debt.remaining')} stroke="#B91C1C" fill="url(#cdRem)" strokeWidth={2} dot={{ r: 3 }} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        )}
        ...
        <div className="pd-tabs-wrap" style={{ padding: 24 }}>
          ...
          {openingBalance > 0 && (
            <div style={{ padding: '10px 14px', background: 'var(--pd-amber-soft)', border: '1px solid #FCD34D', borderRadius: 8, marginBottom: 12, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 8 }}>
              ...
              <strong style={{ fontSize: 15, color: 'var(--pd-red)', fontFamily: 'Geist Mono, monospace' }}>{formatVND(openingBalance)}</strong>
            </div>
          )}
            ...
            summary={() => filteredReceivables.length === 0 ? null : (
              <Table.Summary fixed>
                ...
                <Table.Summary.Row style={{ background: 'var(--pd-bg-warm)' }}>
                  ...
                  <Table.Summary.Cell index={1} align="right"><strong style={{ color: totals.remaining > 0 ? 'var(--pd-red)' : 'var(--pd-green)' }}>{formatVND(totals.remaining)}</strong></Table.Summary.Cell>
                  <Table.Summary.Cell index={2} />
                </Table.Summary.Row>
                <Table.Summary.Row style={{ background: '#FFF5F5', borderTop: '2px solid #ffccc7' }}>
                  ...
                  <Table.Summary.Cell index={1} align="right"><strong style={{ fontSize: 14, color: (openingBalance + totals.remaining) > 0 ? 'var(--pd-red)' : 'var(--pd-green)' }}>{formatVND(openingBalance + totals.remaining)}</strong></Table.Summary.Cell>
                  <Table.Summary.Cell index={2} />
                </Table.Summary.Row>
              </Table.Summary>
            )} />
      ...
      <Modal open={modalData?.type === 'items'} title={`${t('order.productDetails')} - ${modalData?.record?.invoice_number || ''}`} footer={null} width={Math.min(window.innerWidth * 0.95, 700)} onCancel={() => setModalData(null)}>
        {modalData?.type === 'items' && (() => {
          ...
          return (<>
            {rec.sales_order?.expected_delivery && <div style={{ fontSize: 12, marginBottom: 4, color: '#666' }}>{t('order.expectedDelivery')}: {formatDate(rec.sales_order.expected_delivery)}</div>}
            {rec.sales_order?.notes && <div style={{ fontSize: 12, marginBottom: 8, color: '#666' }}>{t('common.notes')}: {rec.sales_order.notes}</div>}
            ...
            ]} summary={() => rec.sales_order?.grand_total != null ? (<Table.Summary.Row><Table.Summary.Cell index={0} colSpan={4} align="right"><strong>{t('order.orderTotal')}</strong></Table.Summary.Cell><Table.Summary.Cell index={1} align="right"><strong style={{ color: '#1F3A8A' }}>{formatVND(rec.sales_order.grand_total)}</strong></Table.Summary.Cell></Table.Summary.Row>) : undefined} />
          </>);
        })()}
      </Modal>

      {showPayment && <PaymentModal open type="receivable" debtId={customerId!} maxAmount={summary.total_remaining} onClose={() => setShowPayment(false)} />}
