...
const ReceivableListPage: React.FC = () => {
  ...
  const { data, isLoading } = useReceivables({
    ...
    customer_search: customer || undefined,
  ...
  const list: Receivable[] = data?.data ?? [];
  ...
  const columns: ColumnsType<Receivable> = [
    ...
    },
    {
      ...
      dataIndex: ['sales_order', 'order_code'],
      key: 'order_code',
      width: 130,
    },
    {
    ...
    },
    {
      ...
      render: (_: unknown, record: Receivable) => (
        <Space size="small">
          {record.sales_order?.id && (
            <Tooltip title={t('common.viewDetail')}>
              <Button type="text" size="small" icon={<EyeOutlined />} style={{ color: '#1677ff' }} onClick={() => navigate(`/sales-orders/${record.sales_order?.id}`)} />
            </Tooltip>
          )}
          {record.remaining > 0 && (
      ...
      <Card style={{ borderRadius: 12 }}>
        ...
        <Space wrap style={{ marginBottom: 16 }}>
          ...
          <Input
            ...
            placeholder={t('debt.searchCustomer')}
            allowClear
            value={customer}
